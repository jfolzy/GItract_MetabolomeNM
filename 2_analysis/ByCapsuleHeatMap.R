...
#Only Select Capsules##########################
#SubsetRows <- which(SampleMetaData$`Sample Type`==GroupOI)
###########################################
...
SampleMetaData[,14:23] <- ifelse(SampleMetaData[,14:23]=="Yes", "aYes", "white")

FVN <- which(colnames(SampleMetaData)==FVariables[2])
...
SampleOrder <- arrange(SampleOrder,SampleOrder[2])
